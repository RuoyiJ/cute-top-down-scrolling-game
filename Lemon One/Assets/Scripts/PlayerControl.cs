...
public class PlayerControl : MonoBehaviour, IPlayerController {
    ...
    void HandleInput()
    {
        ...
        if (IsMoving)
        {
            if (Input.GetKeyDown(KeyCode.D) || (Input.GetMouseButtonDown(0) && Input.mousePosition.x > Screen.width/2))
            {
                if (m_gridIndex < 4)
            ...
            else if (Input.GetKeyDown(KeyCode.A)  || (Input.GetMouseButtonDown(0) && Input.mousePosition.x < Screen.width / 2))
            {
                if (m_gridIndex > 0)
                {//rigid.velocity = new Vector2(-1, 0) * speed * Time.deltaTime;
                    m_gridIndex--;
    ...
    {
        currentLives--;
        StartCoroutine(DisableCollisionForSomeTime(1f));
    }
